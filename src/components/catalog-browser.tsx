            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-1 text-lg font-semibold">{asset.name}</h2>
                </div>
                <span
                  className={`rounded-full px-2.5 py-1 text-xs font-medium ring-1 ${
                    selectedAsset?.id === asset.id
                      ? "bg-white/10 text-white ring-white/20"
                      : statusStyles[asset.certification]
                  }`}
                >
                  {asset.certification}
                </span>
              </div>
              <p className="mt-3 line-clamp-2 text-sm opacity-75">{asset.description}</p>
              <div className="mt-4 flex flex-wrap gap-2">
                {[asset.domain, asset.type, ...asset.tags.slice(0, 2)].map((tag) => (
                  <span
                    key={tag}
